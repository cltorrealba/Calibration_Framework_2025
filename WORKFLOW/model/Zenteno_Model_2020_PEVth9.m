function dxdt = Zenteno_Model_2020_PEVth9(t,x,k,int_time,exp_temp,kfixed)
...
Kd0    = 0.00044;     % 1/h     (Zenteno 2010)

Yxn    = p(9);        % kg Bio/kg N (Zenteno 2010), biomass/nitrogen yield coeff
Yxg    = p(10);       % kg bio/kg G (Zenteno 2010), biomass/glucose yield coeff
Yxf    = p(11);       % kg bio/kg F (zenteno 2010), biomass/fructose yield coeff
Yeg    = p(12);       % kg E/ kg G (Zenteno 2010), ethanol/glucose yield coeff
Yef    = p(13);       % kg E/ kg F (Zenteno 2010), ethanol/fructose yield coeff
...
% Guard small denominators to avoid numerical singularities (0/0, x/0).
den_NKn   = max(N+Kn, eps);
den_GKg   = max(G+Kg, eps);
den_FKf   = max(F+Kf, eps);
den_GKig  = max(G+Kig, eps);
den_EKie  = max(E+Kie, eps);
den_GF    = max(G+F, eps);
% growth rate, 1/h
mu        = mu_max*(N/den_NKn);
% ethanol production rate from glucose
beta_G    = betaG_max*(G/den_GKg)*(Kie/den_EKie);
% ethanol production rate from fructose
beta_F    = betaF_max*(F/den_FKf)*(Kig/den_GKig)*(Kie/den_EKie);
...
dGdt   = -((mu/Yxg)+(beta_G/Yeg)+m*(G/den_GF))*X;    % Glucose
dFdt   = -((mu/Yxf)+(beta_F/Yef)+m*(F/den_GF))*X;    % Fructose
