# -*- coding: utf-8 -*-
"""
Created on Tue May 13 16:06:20 2025

@author: ctorrealba
"""

# re_simulate_zenteno.py
"""
Reimplementation of ReSimulate_Zenteno_Optimal.m and Zenteno_Model_2020_PEVth9.m in Python
"""
...
def resimulate_optimal(k_free, x0, time_dap, int_time, exp_temp,
    ...
    """Two-stage around DAP """
    t1 = int_time[int_time < time_dap]
    t2 = int_time[int_time >= time_dap]

    sol1 = solve_ivp(lambda t, y: zenteno_rhs(t, y, k_free, int_time, exp_temp, kfixed),
                     (t1[0], t1[-1]), x0, t_eval=t1, method='RK45')
    x02 = sol1.y.T[-1].copy()

    x02[1] = Kinetic_Matrix[FDA_add_idx, 3] / 1000.0
    sol2 = solve_ivp(lambda t, y: zenteno_rhs(t, y, k_free, int_time, exp_temp, kfixed),
                     (t2[0], t2[-1]), x02, t_eval=t2, method='RK45')

    T = np.concatenate([sol1.t, sol2.t])
    Xf = np.vstack([sol1.y.T, sol2.y.T])
    return T, Xf
